 ...
 * @returns undefined
 */
var handleHook = function(hooks, type, obj){
  if ( !hooks ) return;
  if ( hooks.trigger ) { type ? hooks.trigger(type, obj) : hooks.trigger(obj); }
  if ( hooks.dispatch ) { type ? hooks.dispatch(type, obj) : hooks.dispatch(obj); }
  if ( hooks.resolve ) { type ? hooks.resolve({type:type, obj:obj}) : hooks.resolve(obj); }
  else if ( hooks.dispatch ) {
    type ? hooks.dispatch(type, obj) : hooks.dispatch(obj);
  }
  else if ( type && hooks[type] ) {
    handleHook(hooks[type], '', obj)
  }
  else if ( hooks.call ) {
    type ? hooks.call(this, type, obj) : hooks.call(this, obj);
  }
};
...
module.exports.compile = function (spec, _extraDefinitions, _hooks) {
  ...
  _.forEach(definitions, function (definition, key) {
    ...
    if (object && !excludedSchema) {
      ...
      handleHook(_hooks, 'schema', schema);
    }
  });
  ...
  _.forEach(schemas, function (schema, key) {
    ...
    handleHook(_hooks, 'model', models[key]);
  });
