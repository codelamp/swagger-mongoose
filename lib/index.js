...
/**
 * Just allows an object with specific hook/callback handling
 * the reason for this is to gain access to the schema objects
 * before models are created from them.
 *
 * @param {Object|Function} hooks - can be of differing structures, either an
 *                          "event handler", callback, deferred, or a signal.
 * @param {String} type - the type of object being emitted
 * @param {Object|Array} obj - the object being emitted
 *
 * @returns undefined
 */
var handleHook = function(hooks, type, obj, name){
  if ( !hooks ) return;
  if ( hooks.trigger ) { type ? hooks.trigger(type, obj, name) : hooks.trigger(obj, name); }
  if ( hooks.dispatch ) { type ? hooks.dispatch(type, obj, name) : hooks.dispatch(obj, name); }
  if ( hooks.resolve ) { type ? hooks.resolve({type:type, obj:obj, name: name}) : hooks.resolve({obj: obj, name: name}); }
  else if ( type && hooks[type] ) {
    handleHook(hooks[type], '', obj, name)
  }
  else if ( hooks.call ) {
    type ? hooks.call(this, type, obj, name) : hooks.call(this, obj, name);
  }
};


module.exports.compileAsync = function (spec, callback, hooks) {
  try {
    callback(null, this.compile(spec, undefined, hooks));
  }
  catch (err) {
...
module.exports.compile = function (spec, _extraDefinitions, _hooks) {
  ...
  _.forEach(definitions, function (definition, key) {
    ...
    if (object && !excludedSchema) {
      ...
      schemas[key] = schema
      // added a call out hook for individual schema extension/modification before model creation
      handleHook(_hooks, 'schema', schema, key);
    }
  });

  // added a call out hook for all schemas before model creation
  handleHook(_hooks, 'schemas', schemas);

  var models = {};
  _.forEach(schemas, function (schema, key) {
    models[key] = mongoose.model(key, schema);
    // added a call out hook for individual models, no so imperative as early schema access
    handleHook(_hooks, 'model', models[key], key);
  });

  // no need for hook 'models' really
  return {
    schemas: schemas,
    models: models
  };
